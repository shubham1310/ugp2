...
#modelfram.add(TimeDistributedDense(output_dim=v2*2))#nb_filter=1000, filter_length= 3,
modelfram.add(TimeDistributedDense(output_dim=500))
...
for iteration in range(1, 20):
    ...
    for i in range(Xtrain.shape[0]):
        if len(Xtrain[i])<=1:
            continue
        Xt = sequence.pad_sequences(Xframtrain[i], maxlen=maxlenfram,padding='post',dtype='float32')
    ...
    for i in range(Xtest.shape[0]):
        if len(Xtest[i])<=1:
            continue
        Xt = sequence.pad_sequences(Xframtest[i], maxlen=maxlenfram,padding='post',dtype='float32')
